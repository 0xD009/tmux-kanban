...
func TestLocalNewSessionArgsAllowsDefaultShell(t *testing.T) {
	got := localNewSessionArgs("scratch", "")
	want := []string{"tmux", "new-session", "-d", "-s", "scratch"}
	if len(got) != len(want) {
		t.Fatalf("args length = %d, want %d: %#v", len(got), len(want), got)
	}
	for i := range want {
...
func TestRemoteNewSessionCommandAllowsDefaultShell(t *testing.T) {
	got := remoteNewSessionCommand("scratch", "")
	want := "tmux new-session -d -s 'scratch'"
	if got != want {
		t.Fatalf("remote command = %q, want %q", got, want)
	}
}

func TestExactSessionTargetRequiresExactName(t *testing.T) {
	if got := exactSessionTarget("work"); got != "=work" {
		t.Fatalf("exact target = %q, want =work", got)
	}
}
